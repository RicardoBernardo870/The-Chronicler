-- Reading Quest Goal
-- Adds per-year reading goals and a single aggregate RPC for Profile quest/level data.
...
create table if not exists public.reading_goals (
  ...
  target_books integer not null check (target_books >= 1),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint reading_goals_user_year_key unique (user_id, year)
);

create or replace function public.set_reading_goals_updated_at()
returns trigger
language plpgsql
...
for each row execute function public.set_reading_goals_updated_at();

-- RLS -----------------------------------------------------------------------
alter table public.reading_goals enable row level security;

drop policy if exists "Reading goals are viewable by owner" on public.reading_goals;
create policy "Reading goals are viewable by owner"
on public.reading_goals for select
to authenticated
using ((select auth.uid()) = user_id);

drop policy if exists "Reading goals are insertable by owner" on public.reading_goals;
create policy "Reading goals are insertable by owner"
on public.reading_goals for insert
to authenticated
with check ((select auth.uid()) = user_id);

drop policy if exists "Reading goals are updatable by owner" on public.reading_goals;
...
drop policy if exists "Reading goals are deletable by owner" on public.reading_goals;
...
create index if not exists progress_history_user_recorded_at_idx
on public.progress_history (user_id, recorded_at desc);

create index if not exists recaps_user_created_at_idx
on public.recaps (user_id, created_at desc);

create index if not exists page_captures_user_captured_at_idx
on public.page_captures (user_id, captured_at desc);

create index if not exists lore_cards_user_created_at_idx
on public.lore_cards (user_id, created_at desc);

-- get_reading_quest_summary -------------------------------------------------
  ...
  return (
    ...
    activity_counts as (
      select
        ...
        (select count(*)::integer from public.page_captures where user_id = p_user_id) as page_captures,
