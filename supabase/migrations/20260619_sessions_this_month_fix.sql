-- =============================================================================
-- File   : 20260619_sessions_this_month_fix.sql
-- Purpose: Fix "Sessions this month" overcounting. The previous definition
--          counted every progress_history row in the last 30 days, including
--          quick page updates made outside a timed reading session. A meaningful
--          reading session is a started session (session_start_at not null) that
--          ran >= 60s and advanced >= 1 page — the same validity filter
--          totalReadingHours uses. Count distinct such sessions instead.
--
-- Based on the live get_reading_stats definition (incl. the `authorized` RLS CTE
-- and the current_progress-based totalPagesRead); only `sessionsThisMonth`
-- changes.
-- =============================================================================
...
as $function$
  ...
  select json_build_object(
    ...
    -- Fixed: count only meaningful reading sessions (started, >= 60s, >= 1 page),
    -- not every progress save, within the last 30 days.
    'sessionsThisMonth',
      coalesce((
