-- =============================================================================
-- File   : 20260621_pages_this_calendar_month.sql
-- Purpose: Make "Pages this month" mean the current CALENDAR month too (matching
--          sessionsThisMonth), instead of a rolling 30-day window. Only the
--          pagesThisMonth date filter changes.
-- =============================================================================
...
as $function$
  ...
  select json_build_object(
    ...
    -- Pages read in the current calendar month.
    'pagesThisMonth',
      coalesce((
        select sum(page_delta) from deltas
    ...
    -- Meaningful reading sessions (started, >= 60s, >= 1 page) in the current
    -- calendar month.
    'sessionsThisMonth',
      coalesce((
