...
- **Focus mode (2026-07):** during an active session, a "Focus mode" button (Book Detail progress panel + Dashboard hero card) opens `FocusModeOverlay.vue` — full-screen cover + big timer + pause/end controls — holding a **Screen Wake Lock** (`src/composables/useWakeLock.ts`; re-acquired on `visibilitychange`, best-effort on unsupported browsers; requires a secure context — plain-HTTP LAN testing has no `navigator.wakeLock`) so the display stays awake beside the reader. After 30 s without a touch it **auto-dims**: a near-black layer fades in (timer faintly visible — OLED-friendly, since the web can't dim-without-locking natively) and any tap wakes it; the dim layer covers the controls so a wake tap can't hit them. Opening/closing never touches session state. **Reading goal (2026-07):** 15/30/45/60-min chips set a display-only countdown from the moment of tap (tap the active chip to clear; pauses with the session since it derives from `elapsedSeconds`); at zero the screen auto-wakes, a synthesized two-ding Web Audio chime plays (AudioContext primed in the chip tap so autoplay rules allow it), Android vibrates, status becomes "Time's up", End Session pulses, and the big display counts overtime as `+M:SS` — the actual session keeps running until the reader ends it. No auto-dim after time's up.
...
- **Reading Stats charts (2026-07):** "Your year" — 12 hand-rolled bars of pages/month with year selector and tap-a-bar detail (`get_monthly_reading` RPC: page deltas + first-finish months from `progress_history`, timezone-aware, organic reads only since imports write no history); segmented **genre bar** in the Library Breakdown card (top 4 + Other, from the existing `genreDistribution`); **Book Lengths** card (average/longest/shortest finished, client-side). Stats page order: Your year → calendar → stats tiles → records → lengths → breakdown. Trophy Room order: quest ring → XP → achievements.
- **Personal records (2026-07):** `get_reading_records(p_user_id, p_timezone)` RPC — best-day pages (same lateral-prev delta logic as `get_monthly_reading`), longest session (12 h outlier cap), fastest finish (first history row → first completion row, in local days), and a night-owl flag (any session ending 00:00–04:59 local); the card adds the all-time longest streak from `get_reading_stats`. Composable `useReadingRecords` (SWR, `readingRecords` cacheKey); card hidden until a record exists.
- **Achievements (2026-07):** catalog + unlock conditions live client-side in `src/utils/achievements.ts` (17 keys drawing only on data the Trophy Room already loads: quest XP sources, lifetime stats, records — no per-achievement queries). The `achievements` table is an **insert-only ledger** of `(user_id, achievement_key, earned_at)` — a trophy never regresses if the underlying stat drops (e.g. a deleted book). `useAchievements.sync()` runs on Trophy Room mount: loads everything, then upserts newly-satisfied keys (`ignoreDuplicates`). `AchievementsGrid.vue` renders earned (lit, dated) vs locked (dimmed, hint visible).
