...
>
> **Session Resume + recap redesign shipped on the PWA (2026-07-13).** Prod now has a **6th edge function, `generate-page-resume`** (verify_jwt true; stateless: capture text → `{ resume: { bullets ≤3, tension } | null }`; the client persists it onto `page_captures.resume` jsonb — migration `20260713_session_page_resume.sql`). Behavior iOS must mirror: (1) tapping Start Session first shows a pre-session "Previously" sheet built from the latest capture's stored resume, and **`session_start_at` is only written when the reader confirms "Begin reading"** — dismissing aborts the start; (2) the sheet is suppressed when the latest recap covers the current position (`page_snapshot >= current_page`) or when no resume exists (strictly resume-or-nothing); (3) resume generation is fire-and-forget after capture save, at most one AI call per capture (persisted; purged with the capture on completion). Separately, **Get Recap now opens a modal** (image-first story layout: illustration slot → corpus/days byline → arc prose → ≤5 watchlist chips → open-thread quote) and recap history cards share that layout (accordions removed); the recap prompt is arc-shaped (deployed v84, repo synced) and the journal header ("pages X–Y · N days later") is computed client-side from the previous recap row.
