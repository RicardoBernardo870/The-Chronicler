...
const page = computed(
  () =>
    graph.searchPages.value[normalizedQuery.value] ?? {
      items: [],
      nextCursor: null,
    },
);
...
<template>
  <section class="reader-search-card glass-surface">
    <header class="reader-search-card__header">
      <div>
        <h2>Search readers</h2>
        <p>Find public readers by username or display name.</p>
      </div>
    </header>

    <span class="reader-search-card__search p-input-icon-left">
      <InputGroup>
        <InputGroupAddon>
    ...
    <Message
      v-if="actionError"
      severity="error"
      class="reader-search-card__message"
    >
      {{ actionError }}
    </Message>

    <p v-if="showEmptyPrompt" class="reader-search-card__hint">
      Type at least 2 characters.
    </p>

    <div
      v-else-if="searching && !page.items.length"
      class="reader-search-card__list"
    >
      <div v-for="n in 3" :key="n" class="reader-search-card__row">
        <Skeleton shape="circle" size="2.5rem" />
        <div class="reader-search-card__copy">
    ...
    <div v-else class="reader-search-card__list">
      <div
        v-for="reader in page.items"
        :key="reader.userId"
        class="reader-search-card__row"
      >
        <Avatar
          ...
          :label="
            reader.avatarUrl
              ? undefined
              : reader.username.slice(0, 1).toUpperCase()
          "
          shape="circle"
          size="large"
        />
        ...
        <button
          type="button"
          class="reader-search-card__identity"
...
.reader-search-card__header h2,
.reader-search-card__header p,
.reader-search-card__hint {
  margin: 0;
}
...
.reader-search-card__search :deep(.p-inputtext) {
  width: 100%;
}

.reader-search-card__message {
  margin: 0;
}
...
.reader-search-card__list {
  display: flex;
  flex-direction: column;
  gap: 0.55rem;
}

.reader-search-card__row {
  display: flex;
  align-items: center;
  gap: 0.7rem;
  min-height: 3.25rem;
...
.reader-search-card__copy,
.reader-search-card__identity {
  min-width: 0;
  flex: 1 1 auto;
}
...
.reader-search-card__identity strong,
.reader-search-card__identity span {
